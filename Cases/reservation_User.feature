Feature: User Reservation
  ...
  Scenario: Create new reservation
    Given the user with number "1" is logged in
  ...
  Scenario: Check reservation status
    Given the user with number "1" is checking the status of reservation number "6"
    Then the reservation status is displayed

  Scenario: Receive reservation and payment
    Given the user with number "1" wants to receive his reservation with number "3" in a ready state
    When the user with number "1" requests to receive reservation number "3" and pay the debt
